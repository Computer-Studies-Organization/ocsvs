...
function createHooks() {
  ...
  return {
    useState<T>(initial: T): [T, (v: T) => void] {
      const i = callIndex++;
      if (stateSlots[i] === undefined) stateSlots[i] = initial;
      return [
        stateSlots[i] as T,
        (v: T) => {
          stateSlots[i] = v;
    ...
    useRef<T>(initial: T): { current: T } {
      const i = callIndex++;
      if (!refSlots[i]) refSlots[i] = { current: initial };
...
test("setTimeout cleanup — timeout cleared on unmount", async () => {
  ...
  const result = useMutationWithFeedback({ mutateAsync: () => Promise.resolve() } as any, {
    successMessage: "done",
    autoCloseMs: 100,
    onSuccess,
  });
