<script lang="ts">
  ...
  import { fetchUsers, updateUser, deleteUser, restoreUser } from '$lib/api/users'
  ...
  $effect(() => { includeDeleted; loadUsers() })
  ...
  let paginated = $derived(filtered.slice(pageIndex * pageSize, (pageIndex + 1) * pageSize))
