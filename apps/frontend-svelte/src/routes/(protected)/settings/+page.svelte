<script lang="ts">
  ...
  import { getMyProfile, updateMyProfile, changePassword } from '$lib/api/profile'
  import type { ProfileData, UpdateProfileData, ChangePasswordData } from '$lib/types'
  ...
  let isAdmin = $derived(user?.user?.role === UserRole.ADMIN)
  ...
  async function handleProfileSubmit(e: SubmitEvent) {
    ...
    try {
      ...
      setTimeout(() => { profileMsg = null }, 4000)
    }
  }
  ...
  async function handlePasswordSubmit(e: SubmitEvent) {
    ...
    try {
      ...
      setTimeout(() => { passwordMsg = null }, 4000)
    }
  }
</script>
