<script lang="ts">
  ...
  import { YEAR_LEVEL_VALUES, COURSE_VALUES } from '$lib/types'
  ...
  let isLoginValid = $derived(loginData.studentNumber.trim().length > 0 && loginData.password.trim().length > 0)
  let isStepOneValid = $derived(isRegisterUserDraftStepOneComplete(registerData))
  let isRegisterValid = $derived(isRegisterUserDraftComplete(registerData))
