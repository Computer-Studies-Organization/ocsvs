...
test.describe('Authentication - Session Lifecycle', () => {
  test('rejects unauthenticated session check', async ({ request }) => {
    const response = await request.get('http://localhost:8787/auth/me');
  ...
  test('maintains session across authenticated requests and handles logout', async ({ request }) => {
    ...
    // 2. Fetch /auth/me with session cookie
    const meRes = await request.get('http://localhost:8787/auth/me', {
    ...
    // 3. Logout
    const logoutRes = await request.post('http://localhost:8787/auth/logout', {
      headers: { Cookie: cookies },
    });
    expect(logoutRes.ok()).toBe(true);

    // 4. Verify session is revoked
    const mePostLogout = await request.get('http://localhost:8787/auth/me', {
      headers: { Cookie: cookies },
    });
