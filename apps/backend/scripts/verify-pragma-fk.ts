...
async function main() {
  ...
  console.log('SETUP: create parent(id) and child(id, parent_id REFERENCES parent(id) ON DELETE CASCADE)')
  ...
  console.log(`  → children remaining after parent delete: ${n} ${n === 0 || n === 0n ? '(CASCADE fired — PASS)' : '(CASCADE did NOT fire — FAIL)'}`)
