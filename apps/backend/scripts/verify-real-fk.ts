...
async function main() {
  ...
  async function tryExpect(label: string, fn: () => Promise<unknown>, shouldFail: boolean) {
    try {
      await fn()
      if (shouldFail)
        console.log(`  ❌ ${label} — succeeded, expected rejection`)
  ...
  const remainPos = await client.execute(`SELECT count(*) AS n FROM positions WHERE id = '${P}-p1'`)
  const n1 = Number((remainPos.rows[0] as Row).n)
  console.log(`  ${n1 === 0 ? '✅' : '❌'} cascade: ${n1} position(s) remain after parent delete (expected 0)`)
