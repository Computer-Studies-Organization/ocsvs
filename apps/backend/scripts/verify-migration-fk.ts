...
async function setupLegacyTables(client: ReturnType<typeof createClient>, seed: boolean) {
  ...
  if (seed) {
    ...
    const vid = "vote-1";
    await client.execute({
      sql: `INSERT INTO accounts(id, username, password_hash, role, created_at, updated_at, last_login) VALUES (?, 'admin', 'hash', 'admin', ?, ?, ?)`,
      args: [aid, now, now, now],
    });
    await client.execute({
      sql: `INSERT INTO users(id, account_id, student_id, first_name, last_name, course, year_level, created_at, updated_at) VALUES (?, ?, 'S001', 'A', 'B', 'BSCS', '4th', ?, ?)`,
      args: [uid, aid, now, now],
    });
    await client.execute({
      sql: `INSERT INTO candidates(id, full_name, account_id, position, manifesto, created_at, updated_at) VALUES (?, 'Alice', ?, 'President', 'hi', ?, ?)`,
      args: [cid, aid, now, now],
    });
    await client.execute({
      sql: `INSERT INTO votes(id, user_id, candidate_id, position, created_at, updated_at) VALUES (?, ?, ?, 'President', ?, ?)`,
      args: [vid, uid, cid, now, now],
    });
  }
}
...
async function main() {
  ...
  console.log("--- M1 / M2: ALTER TABLE ADD NOT NULL REFERENCES on empty vs non-empty table ---");
  {
    ...
    const m2 = await tryStep("M2: same ALTER — non-empty table", () =>
      client.execute(`ALTER TABLE child ADD parent_id TEXT NOT NULL REFERENCES parent(id)`),
    );
    if (!seedExisting.ok) {
      console.log(
        `      ↳ existing-row insert failed (NOT NULL col already has no default on empty row? unusual)`,
      );
    }
    if (!m2.ok) {
      console.log(
        `      ↳ expected on SQLite: ALTER rejected because existing rows lack a default for the NOT NULL col`,
      );
    }
  }
  ...
  console.log("\n--- M3: FK enforcement after ALTER (when it succeeded) ---");
  {
    ...
    const m3 = await tryStep("M3: INSERT child referencing non-existent parent is rejected", () =>
      client.execute(`INSERT INTO child(id, parent_id) VALUES ('c1', 'ghost')`),
    );
    if (m3.ok) console.log("      ↳ UNEXPECTED: FK was not enforced after ALTER");
  }
  ...
  console.log(`  migration has ${statements.length} statements`);
  {
    ...
    console.log(
      `  → M4 result: ${passCount}/${statements.length} statements applied${failCount === 0 ? " — PASS" : " — FAIL"}`,
    );

    // Post-migration sanity: does FK enforcement actually work on the new schema?
    const seedElection = await tryStep("M4-post: seed election e1 + position p1", async () => {
      await client.execute(
        `INSERT INTO elections(id, name, status) VALUES ('e1', 'Test', 'draft')`,
      );
      await client.execute(
        `INSERT INTO positions(id, election_id, name) VALUES ('p1', 'e1', 'Prez')`,
      );
    });
    if (seedElection.ok) {
      const orphanPos = await tryStep(
        ...
        () =>
          client.execute(
            `INSERT INTO positions(id, election_id, name) VALUES ('p-bad', 'ghost', 'X')`,
          ),
      );
      if (orphanPos.ok) console.log("      ↳ UNEXPECTED: FK not enforced on positions.election_id");
    } else {
  ...
  );
  {
    ...
    if (failAt === -1) {
      console.log(
        "  → M5 result: all statements applied (migration is NOT destructive on populated DB — contradicts header)",
      );
    } else {
      console.log(
        `  → M5 result: migration stops at stmt #${failAt} when data is present (matches header: wipe first)`,
      );
    }
  }
