...
export const listElectionsHandler: AppRouteHandler<typeof listElectionsRoute> = async (c) => {
  ...
  return c.json(await electionRepo.list(db, status ? { status: status as TElectionStatus } : undefined), httpStatusCodes.OK)
}
...
export const getCurrentElectionHandler: AppRouteHandler<typeof getCurrentElectionRoute> = async (c) => {
  const { db } = createDb(c)
...
export const transitionElectionHandler: AppRouteHandler<typeof transitionElectionRoute> = async (c) => {
  if (c.var.authUser?.role !== 'admin') {
    return c.json({ message: ERROR_MESSAGES.FORBIDDEN }, httpStatusCodes.FORBIDDEN)
  }
  const { db } = createDb(c)
  ...
  await electionRepo.updateStatus(db, id, { status: to, opensAt: opensAt ?? null, closesAt: closesAt ?? null })
  const messageKey = (
