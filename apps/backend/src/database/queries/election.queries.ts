...
export const electionQueries = {
  ...
  async getElectionWithPositions(db: Database, id: string): Promise<ElectionWithPositions | null> {
    ...
    if (!election)
      return null
    const positionRows = await db.select().from(positions).where(eq(positions.electionId, id)).orderBy(asc(positions.displayOrder), asc(positions.createdAt)).all()
    ...
    return {
      ...
      positions: positionRows.map(p => ({
        ...
        candidates: candidateRows.filter(c => c.positionId === p.id)
          .map(c => ({ id: c.id, fullName: c.fullName, isActive: c.isActive, manifesto: c.manifesto })),
      })),
    }
  },

  async countPositions(db: Database, electionId: string): Promise<number> {
    const row = await db.select({ count: count() }).from(positions).where(eq(positions.electionId, electionId)).get()
  ...
  async getResults(db: Database, electionId: string): Promise<ResultsPosition[]> {
    ...
    for (const r of rows) {
      ...
      if (r.candidateId) {
        slot.candidates.push({ candidateId: r.candidateId, fullName: r.candidateName ?? '', voteCount: r.voteCount, percentage: 0 })
      }
    }
