...
/**
 * GET /users/{id}/audit — full audit trail for a single user.
 * Gated by `requireAdmin` at the registration site in step 7.
 */
export const getUserAuditRoute = createRoute({
  method: "get",
  path: "/users/{id}/audit",
  tags: ["Users", "Audit Log"],
  summary: "Get audit trail for a user (admin only)",
