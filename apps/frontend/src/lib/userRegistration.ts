...
export function getRegisterUserDraftStepOneValidationMessage(user: Pick<TRegisterUserDraft, 'studentId' | 'firstName' | 'lastName'>) {
  const studentId = user.studentId.trim()
  if (!studentId)
    return 'Student ID is required'
  if (!STUDENT_ID_PATTERN.test(studentId))
    return 'Invalid Student ID format (e.g. C25-01-10306-MAN121)'
...
export function isRegisterUserDraftStepOneComplete(user: Pick<TRegisterUserDraft, 'studentId' | 'firstName' | 'lastName'>) {
  return getRegisterUserDraftStepOneValidationMessage(user) === null
}

export function getRegisterUserDraftValidationMessage(user: TRegisterUserDraft) {
...
export function isRegisterUserDraftComplete(user: TRegisterUserDraft): user is TRegisterUser {
  return getRegisterUserDraftValidationMessage(user) === null
}

export function getMutationErrorMessage(error: unknown, fallbackMessage: string, fieldLabels: Record<string, string> = {}) {
