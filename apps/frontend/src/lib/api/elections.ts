...
export async function createElection(body: { name: string, description?: string, opensAt?: number, closesAt?: number }): Promise<TElection> {
  return apiFetch<TElection>('/elections', { method: 'POST', body: JSON.stringify(body) })
}

export async function updateElection(id: string, body: Partial<{ name: string, description: string, opensAt: number, closesAt: number }>): Promise<TElection> {
  return apiFetch<TElection>(`/elections/${id}`, { method: 'PATCH', body: JSON.stringify(body) })
}

export async function transitionElection(id: string, body: { to: TElectionStatus, opensAt?: number, closesAt?: number }): Promise<{ message: string }> {
  return apiFetch<{ message: string }>(`/elections/${id}/transitions`, { method: 'POST', body: JSON.stringify(body) })
}
