...
{#if density === 'compact'}
  <!-- Compact Horizontal Media-Object Layout -->
  <div
    class="group relative flex w-full flex-row gap-3 sm:gap-4 rounded-2xl border p-3 sm:p-4 text-left transition-all duration-200 overflow-hidden active:scale-[0.99] sm:active:scale-100 {selected ? 'border-blue-500 bg-blue-950/25 shadow-lg shadow-blue-500/10 ring-1 ring-blue-500/50' : 'bg-slate-900/40 border-slate-800/80 hover:border-slate-700/80 hover:bg-slate-900/60 hover:-translate-y-0.5 hover:shadow-xl'} focus-within:outline-none focus-within:ring-2 focus-within:ring-blue-500/50"
  >
    <button
      type="button"
    ...
    <!-- Compact Left Section: Framed Portrait / Silhouette Thumbnail -->
    <div class="relative z-10 pointer-events-none w-20 sm:w-28 shrink-0 aspect-[3/4] overflow-hidden rounded-xl border border-slate-800/80 bg-slate-950 flex items-center justify-center shadow-inner self-start">
      ...
      {:else}
        ...
        <div class="flex h-full w-full items-center justify-center bg-gradient-to-b from-slate-900/50 to-slate-950 p-2 sm:p-3">
          <svg
            viewBox="0 0 24 24"
            fill="none"
            class="h-full w-full text-slate-700 transition-transform duration-300 group-hover:scale-[1.02]"
            aria-hidden="true"
    ...
    <!-- Compact Right Section: Details, Manifesto & Actions -->
    <div class="relative z-10 pointer-events-none flex flex-1 min-w-0 flex-col justify-between">
      <div>
        <!-- Header row: Name + Party badge + Checkmark -->
        <div class="flex items-start justify-between gap-2">
          <div class="flex flex-col gap-1 min-w-0 flex-1">
            <div class="flex items-center gap-1.5 flex-wrap">
              <h3 class="font-bold text-slate-100 text-sm sm:text-base group-hover:text-white transition-colors tracking-tight">
                {candidate.fullName}
              </h3>
              <div>
                {#if party}
                  {#if electionId}
                    <a
                      ...
                      class="relative z-20 min-h-11 inline-flex pointer-events-auto items-center text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full border shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 active:scale-95 transition hover:scale-105"
                  ...
                  {/if}
                {:else}
                  <span class="inline-flex items-center text-[9px] font-mono font-bold px-2 py-0.5 rounded-full border border-slate-700/80 bg-slate-800/60 text-slate-400">
                    INDEPENDENT
                  </span>
                {/if}
              </div>
          ...
          <div
            class="flex h-6 w-6 sm:h-7 sm:w-7 shrink-0 items-center justify-center rounded-full border backdrop-blur-md transition-all duration-300 {selected ? 'border-blue-400 bg-blue-500 text-white scale-110 shadow-[0_0_10px_rgba(59,130,246,0.6)]' : 'border-slate-700 bg-slate-950/80 text-transparent scale-100 group-hover:border-slate-600'}"
          >
        ...
        <!-- Platform Manifesto -->
        <div class="mt-2 w-full border-t border-white/5 pt-1.5">
          <p class="text-slate-300 text-xs font-normal leading-relaxed transition-all duration-200 {isExpanded ? '' : 'line-clamp-2'}">
            {candidate.manifesto || 'No platform manifesto provided for this candidate.'}
          </p>
          {#if candidate.manifesto && candidate.manifesto.length > 90}
            <button
              type="button"
              class="relative z-20 min-h-11 pointer-events-auto text-blue-400 hover:text-blue-300 font-semibold cursor-pointer underline text-[10px] uppercase focus:outline-none focus:ring-1 focus:ring-blue-500/50 rounded inline-flex items-center pt-0.5"
              onclick={(e) => {
                e.stopPropagation();
      ...
      <div class="mt-2 flex items-center justify-between gap-2 border-t border-white/5 pt-1.5">
        <span class="text-[10px] font-mono {selected ? 'text-blue-400 font-semibold' : 'text-slate-400'}">
          {selected ? '● Selected on ballot' : '○ Click to select'}
        </span>
...
{:else}
  <!-- Detailed Campaign Poster Layout -->
  <div
