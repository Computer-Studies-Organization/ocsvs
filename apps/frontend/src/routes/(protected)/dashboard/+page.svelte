<script lang='ts'>
  ...
  const positionGroups = $derived<TPositionGroup[]>((() => {
  ...
  const currentGroup = $derived(positionGroups[currentPositionIndex])
  const isFirst = $derived(currentPositionIndex === 0)
  const isLast = $derived(currentPositionIndex === positionGroups.length - 1)
  const hasCurrentVote = $derived(currentGroup ? selectedVotes[currentGroup.id] !== null : false)
  const completedCount = $derived(Object.values(selectedVotes).filter(Boolean).length)
  const allVoted = $derived(positionGroups.every(g => selectedVotes[g.id] !== null))
